...
(()=>{
  const T=THREE; let spawned=false, boss=null, arena=null;
  const M=(c,e=0)=>new T.MeshStandardMaterial({color:c,emissive:e,emissiveIntensity:e?.25:0,roughness:.7});
  function build(){
    if(spawned||state.quest<8)return; spawned=true;
    ...
    scene.add(boss); enemies.push(boss); setQuest('Chapter II · Face the Aegean Champion.'); msg('AEGEAN CHAMPION · ENTER THE ARENA');
  }
  function update(){
    if(state.quest<8)return; build(); if(!boss||boss.userData.dead)return;
